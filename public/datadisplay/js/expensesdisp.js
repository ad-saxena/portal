...
//    }
  ...
  function getdata(){
  var playersRef = firebase.database().ref("DailyExpenses/");
      playersRef.on("child_added", function(data) {
      var mokData = [data.val()] ;
      console.log(mokData);
      $.each(mokData, function (i) {
        templateString = '<tr> <th scope="row">*</th> <td>'+mokData[i].Name+'</td> <td>'+mokData[i].Surname+'</td> <td>'+mokData[i].Department+'</td> <td>'+mokData[i].Amount+'</td> <td>'+mokData[i].Date+'</td> <td>'+mokData[i].EntryPerson+'</td> <td>'+mokData[i].Purpose+'</td> </tr>';
  ...
  function totalt(){
    $(document).ready(function() {
      $('table thead th').each(function(i=3) {
          calculateColumn(i=3);
      });
  });
