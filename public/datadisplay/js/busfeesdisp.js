...
//    }
  ...
  function getdata(){
  var playersRef = firebase.database().ref("BusFees/");
      playersRef.on("child_added", function(data) {
      var mokData = [data.val()] ;
      console.log(mokData);
      $.each(mokData, function (i) {
        templateString = '<tr> <th scope="row">*</th> <td>'+mokData[i].Name+'</td> <td>'+mokData[i].FathersName+'</td> <td>'+mokData[i].Class+'</td> <td>'+mokData[i].Medium+'</td><td>'+mokData[i].Bus+'</td> <td>'+mokData[i].Amount+'</td> <td>'+mokData[i].Date+'</td> <td>'+mokData[i].EntryPerson+'</td> <td>'+mokData[i].Remark+'</td> </tr>';
  ...
  function totalt(){
    $(document).ready(function() {
      $('table thead th').each(function(i=5) {
          calculateColumn(i=5);
      });
  });
